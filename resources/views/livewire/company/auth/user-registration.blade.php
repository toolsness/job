<main class="flex items-center justify-center px-4 py-8 bg-white">
    <section class="w-full max-w-2xl">
        <h1 class="mb-8 text-xl font-semibold text-center text-black">Company Representative Registration</h1>
        <form wire:submit.prevent="checkErrorsAndSubmit" class="space-y-6">
            @foreach (['username', 'companyId', 'name', 'nameKanji', 'nameKatakana', 'email', 'contactPhoneNumber'] as $field)
                <div class="flex flex-col sm:flex-row">
                    ...
                    <div class="flex-grow sm:w-2/3">
                        @if ($field === 'companyId')
                            <select id="{{ $field }}" wire:model.lazy="{{ $field }}"
                    ...
                    <div class="flex-grow sm:w-2/3">
                        <div class="relative">
                            ...
                            <button type="button" @click="show{{ ucfirst($field) }} = !show{{ ucfirst($field) }}"
                                ...
                                <i class="fas"
                                    :class="{ 'fa-eye': !show{{ ucfirst($field) }}, 'fa-eye-slash': show{{ ucfirst($field) }} }"></i>
                            </button>
                        </div>
                        @error($field)
                            <span class="text-sm text-red-500">{{ $message }}</span>
            ...
            <div class="flex flex-col items-center mt-8">
                ...
                <div class="flex flex-col justify-between w-full mt-8 sm:flex-row">
                    ...
                    <button type="button"
                        onclick="window.location='{{ route('company.new-member-registration') }}'"
